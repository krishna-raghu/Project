...
public class SecurityConfig {
    ...
    @Autowired
    private CorsFilter corsFilter; // Automatically injects the custom bean from your CorsConfig.java
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/v1/public/**").permitAll() // Open/public routes
                        .anyRequest().authenticated()                    // Secure everything else!
                )

                // 4. Enable the OAuth2 Resource Server to automatically read Supabase JWTs
                .oauth2ResourceServer(oauth2 -> oauth2
                        .jwt(jwt -> {}) // Uses the config from your application.properties
                );
